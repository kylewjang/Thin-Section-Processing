...
dat.save('C:/Users/kwjan/OneDrive/Documents/resized_andesite.jpg')
dat = plt.imread('C:/Users/kwjan/OneDrive/Documents/resized_andesite.jpg')
...
filtdat = np.multiply(filtdat, 1 / np.max(filtdat))
...
for index in np.arange(np.shape(get_input)[0]):
    ...
    colour = filtdat[np.round(xcoord), np.round(ycoord)]
    print("The corresponding colour values are ", colour)
    for j in np.arange(np.shape(filtdat)[0]):
        for k in np.arange(np.shape(filtdat)[1]):
            check1 = np.all(np.abs(filtdat[j, k] - colour) <= [check1size*2, check1size*2, check1size*2])
            check2 = np.logical_or(np.any(np.logical_and(filtdat[j, k] != 0, colour != 0)),
                                   np.all(np.logical_and(filtdat[j, k] == 0, colour == 0)))
            if np.logical_and(check1, check2):
                counter = counter + 1
